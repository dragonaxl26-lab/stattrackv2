...
def _parse_profile(profile: Dict[str, Any]) -> Tuple[str, Optional[int], Optional[int], str]:
    torn_name = _safe_str(_first_present(profile, [("name",), ("player_name",)], default=""))
    torn_id = _safe_int(_first_present(profile, [("player_id",), ("id",)], default=0), 0) or None
    faction_blob = _first_present(profile, [("faction",)], default={})
...
def _parse_battlestats(payload: Dict[str, Any]) -> PlayerStats:
    return PlayerStats(
        strength=_safe_float(_first_present(payload, [("strength",)], default=0.0), 0.0),
        speed=_safe_float(_first_present(payload, [("speed",)], default=0.0), 0.0),
        defense=_safe_float(_first_present(payload, [("defense",)], default=0.0), 0.0),
        dexterity=_safe_float(_first_present(payload, [("dexterity",)], default=0.0), 0.0),
    )


def _parse_recovery_state(bars: Dict[str, Any], cooldowns: Dict[str, Any]) -> RecoveryState:
    energy_blob = _first_present(bars, [("energy",)], default={})
    happy_blob = _first_present(bars, [("happy",)], default={})
    ...
    return RecoveryState(
        ...
        refill_energy=150,
        drug_cd_minutes=_coerce_cooldown_minutes(_first_present(cooldowns, [("drug",)], default=0)),
        booster_cd_minutes=_coerce_cooldown_minutes(_first_present(cooldowns, [("booster",)], default=0)),
...
def fetch_player_state_from_api(api_key: str, manual_unlocked_gyms: Optional[List[str]] = None) -> PlayerState:
    ...
    if faction_war_days:
        api_notes.append(f"Loaded {len(faction_war_days)} war/non-training dates from faction endpoints.")
    else:
        api_notes.append("No war dates were detected from faction endpoints.")
